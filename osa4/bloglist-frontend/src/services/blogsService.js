...
const postNewBlog = async (blog) => {
  const res = await axios.post(baseUrl, blog).catch((error) => console.log('error', error));
  return res.data;
};
...
const deleteBlog = async (blogId) => {
  const res = await axios.delete(`${baseUrl}/${blogId}`).catch((error) => console.log('error', error));
  return res;
};
