...
test("new blog form tests", async () => {
  ...
  expect(mockHandler.mock.calls[0][0].target.elements.title.value).toBe(
    "Test title...",
  );
  expect(mockHandler.mock.calls[0][0].target.elements.author.value).toBe(
    "Test author...",
  );
  expect(mockHandler.mock.calls[0][0].target.elements.url.value).toBe(
    "https://test.test",
  );
});
